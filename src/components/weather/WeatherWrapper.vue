        ...
        <div class="clearfix" v-if="typeof result.main != 'undefined'">
          ...
          <div class="weather-result" >
            <div class="weather-location">
              <span class="heading">{{ result.name }}, {{ result.sys.country }}</span>
            </div>
...
export default {
  ...
  created() {
    let long;
    let lat;
    if (navigator.geolocation) { // Kiểm tra supported
      navigator.geolocation.getCurrentPosition(position => {
        long = position.coords.longitude
        lat = position.coords.latitude
        fetch(`${this.$store.state.url}/weather?q=${this.$store.state.query}&lat=${lat}&lon=${long}&units=metric&lang=vi&appid=${this.$store.state.key_api}`)
          .then( respon => {
            return respon.json()
          })
          .then( data => {
  ...
  methods: {
    fetchWeather() {
      if(this.$store.state.query.trim() != '') {
        fetch(`${this.$store.state.url}/weather?q=${this.$store.state.query}&units=metric&lang=vi&appid=${this.$store.state.key_api}`)
          .then( respon => {
            return respon.json()
          })
          .then( data => {
